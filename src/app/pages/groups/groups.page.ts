...
export class GroupsPage implements OnInit {
	...
	getGroups() {
		this.groupService.getGroups(this.authUser.uid).subscribe(response => {
			setTimeout(() => (this.groups = response), 1200);
		});
	}
			...
			buttons: [
				{
					...
					handler: () => {
						this.groupService
							.removeMember(group.id, this.authUser.uid)
							.then(response => {
								if (group.createdBy === this.authUser.uid) {
									this.groupService
										.assignNewCreator(group)
										.then(() => this.getGroups());
								}
							});
					}
